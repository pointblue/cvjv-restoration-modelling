...
# tidal frame---------
# limit prioritization to tidal wetlands
tidlmask = rast('SDM_predictors/cover/tidal/baseline21/TIDL.tif') |>
  subst(from = 0, to = NA)
predmaps_tidl = list.files('SDM_results/tidal/v10/', 'NARM.tif$', full.names = TRUE) |>
  rast() |> mask(tidlmask)
plot(predmaps_tidl)
writeRaster(predmaps_tidl,
            paste0('SDM_results/tidal/v10/', names(predmaps_tidl), '_tidal.tif'))

# project set up-----------
spp_list = paste0('~/A_Projects/delta_tima_spp/',
                  list.files('SDM_results/tidal/v10/', '_tidal.tif$', full.names = TRUE)) |>
...
spp_list |>
  ...
  write_delim('zonation/features_equal.txt')
write_file(x = 'feature list file = features_equal.txt',
           file = 'zonation/settings_equal.z5')

# secretive marsh birds only
spp_list |> filter(grepl('BLRA|AMBI|LEBI', filename)) |>
  mutate(weight = 1.0) |>
  select(weight, filename) %>%
  write_delim('zonation/features_secretive.txt')
write_file(x = 'feature list file = features_secretive.txt',
           file = 'zonation/settings_secretive.z5')

# songbirds only
spp_list |> filter(!grepl('BLRA|AMBI|LEBI', filename)) |>
  mutate(weight = 1.0) |>
  select(weight, filename) %>%
  write_delim('zonation/features_songbirds.txt')
write_file(x = 'feature list file = features_songbirds.txt',
           file = 'zonation/settings_songbirds.z5')
...
z5 -w --mode=CAZ1 --gui settings_secretive.z5 caz1_secretive_out
z5 -w --mode=CAZ2 --gui settings_secretive.z5 caz2_secretive_out
z5 -w --mode=CAZ1 --gui settings_songbirds.z5 caz1_songbirds_out
z5 -w --mode=CAZ2 --gui settings_songbirds.z5 caz2_songbirds_out

@pause',
file = 'zonation/z5_run.cmd')
# --> run the cmd file jus t by double-clicking on it. It will open the Zonation
